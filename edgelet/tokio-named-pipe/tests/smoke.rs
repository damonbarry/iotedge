...
fn read_data() {
    ...
    let (_, read_data, read_len) = tokio::runtime::current_thread::Runtime::new().unwrap().block_on(task).unwrap();
...
fn write_data() {
    ...
    tokio::runtime::current_thread::Runtime::new().unwrap().block_on(write_future).unwrap();
...
fn read_async() {
    ...
    let result = tokio::runtime::current_thread::Runtime::new().unwrap().block_on(task).unwrap();
    assert_eq!(
        result,
