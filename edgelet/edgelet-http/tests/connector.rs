...
fn hello_handler(_: Request<Body>) -> impl Future<Item = Response<Body>, Error = HyperError> {
    ...
    response.headers_mut().typed_insert(&ContentLength(GET_RESPONSE.len() as u64));
    future::ok(response)
}
...
fn tcp_get() {
    ...
    let server =
        run_tcp_server("127.0.0.1", port, hello_handler)
...
fn uds_get() {
    ...
    let server =
        run_uds_server(
            &path_copy,
            |req| hello_handler(req).map_err(|err| io::Error::new(io::ErrorKind::Other, err)),
        )
        .map_err(|err| eprintln!("{}", err));

    let connector = UrlConnector::new(&Url::parse(&format!("unix://{}", file_path)).unwrap()).unwrap();
...
fn post_handler(req: Request<Body>) -> Box<Future<Item = Response<Body>, Error = HyperError> + Send> {
...
fn tcp_post() {
    let port = get_unused_tcp_port();
    let server =
        run_tcp_server("127.0.0.1", port, post_handler)
        .map_err(|err| eprintln!("{}", err));
    ...
    *req.uri_mut() = url;
    ...
    req.headers_mut().typed_insert(&ContentLength(POST_BODY.len() as u64));
