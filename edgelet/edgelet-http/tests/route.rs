...
fn route1(_req: Request<Body>, params: Parameters) -> Box<Future<Item = Response<Body>, Error = HyperError> + Send> {
    let response = params
        .name("name")
        .map(|name| {
            Response::builder()
...
fn route2(_req: Request<Body>, params: Parameters) -> Box<Future<Item = Response<Body>, Error = HyperError> + Send> {
    let response = params
        .name("name")
        .map(|name| {
            Response::builder()
