...
use error::Error;
use futures::future;
use hyper::{Body, Client as HyperClient, Error as HyperError, Request, Response, StatusCode, Uri, client::HttpConnector};
        ...
        if self.null {
            ...
            match config.proxy_uri {
                ...
                Some(uri) => {
                    ...
                    Ok(Client::Proxy(
                        HyperClient::builder().build(conn),
                    ))
                }
            }
        }
    ...
    fn call(&self, req: Request<Body>) -> Self::Response {
        match *self {
            ...
            Client::Null => Box::new(future::ok(
                Response::builder()
                .status(StatusCode::from_u16(234).expect("StatusCode::from_u16 should not fail"))
