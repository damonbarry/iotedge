...
use hyper::{Uri, client::HttpConnector, client::connect::{Connect, Connected, Destination}};
#[cfg(windows)]
...
impl Connect for UrlConnector {
    ...
    fn connect(&self, dst: Destination) -> Self::Future {
        ...
        match self {
            UrlConnector::Http(connector) => Box::new(
                connector
                    .connect(dst)
                    .and_then(|(tcp_stream, connected)| Ok((StreamSelector::Tcp(tcp_stream), connected))),
            ) as Self::Future,

            #[cfg(windows)]
            UrlConnector::Pipe(connector) => Box::new(
                connector
                    .connect(dst)
            ...
            UrlConnector::Unix(connector) => Box::new(
                connector
                    .connect(dst)
                    .and_then(|(unix_stream, connected)| Ok((StreamSelector::Unix(unix_stream), connected))),
            ) as Self::Future,
        }
    }
}
...
mod tests {
    ...
    fn create_http_succeeds() {
        let _connector =
            UrlConnector::new(&Url::parse("http://localhost:2375").unwrap()).unwrap();
    }
    ...
    fn create_pipe_succeeds() {
        let _connector =
            UrlConnector::new(&Url::parse("npipe://./pipe/boo").unwrap()).unwrap();
    }
}
