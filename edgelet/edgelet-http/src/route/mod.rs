...
pub trait Handler<P>: 'static + Send {
    fn handle(&self, req: Request<Body>, params: P) -> Box<Future<Item = Response<Body>, Error = hyper::Error> + Send>;
}

impl<F, P> Handler<P> for F
where
    F: 'static + Fn(Request<Body>, P) -> Box<Future<Item = Response<Body>, Error = hyper::Error> + Send> + Send,
{
    fn handle(&self, req: Request<Body>, params: P) -> Box<Future<Item = Response<Body>, Error = hyper::Error> + Send> {
        (*self)(req, params)
    }
