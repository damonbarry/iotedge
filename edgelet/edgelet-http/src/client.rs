...
use serde_json;
use typed_headers::{ContentLength, ContentType, HeaderMapExt, http, mime};
...
impl<C> ClientImpl for hyper::Client<C, Body> where
...
impl<F, R> ClientImpl for F where
    ...
    T::Error: Into<Error>,
{
    ...
    fn add_sas_token(&self, req: &mut Request<Body>, path: &str) -> Result<(), Error> {
        if let Some(ref source) = self.token_source {
            ...
            req.headers_mut()
                .append(http::header::AUTHORIZATION, format!("SharedAccessSignature {}", token).parse().unwrap());
        } else {
...
mod tests {
    ...
    use tokio;
    use typed_headers::{ContentType, mime};
    ...
    fn request_adds_api_version() {
        ...
        let client =
            Client::new(handler, token_source, api_version, host_name).unwrap();

        let task = client.request::<String, String>(Method::GET, "/boo", None, None, false);

        let _result: Option<String> = tokio::runtime::current_thread::Runtime::new().unwrap().block_on(task).unwrap();
    }
    ...
    fn request_adds_api_version_with_other_query_params() {
        ...
        let _result: String = tokio::runtime::current_thread::Runtime::new().unwrap().block_on(task).unwrap().unwrap();
    }
    ...
    fn request_adds_if_match_header() {
        ...
        let handler = move |req: Request<Body>| {
            assert_eq!(Some("*").map(AsRef::as_ref), req.headers().get(hyper::header::IF_MATCH).map(AsRef::as_ref));
    ...
    fn request_can_return_empty_response() {
        ...
        let result: Option<String> = tokio::runtime::current_thread::Runtime::new().unwrap().block_on(task).unwrap();
    ...
    fn request_returns_response() {
        ...
        let result: String = tokio::runtime::current_thread::Runtime::new().unwrap().block_on(task).unwrap().unwrap();
