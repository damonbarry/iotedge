...
mod tests {
    ...
    fn server_register_with_auth_success() {
        ...
        let handler = move |req: Request<Body>| {
            let (http::request::Parts { method, uri, headers, .. }, _body) = req.into_parts();
            ...
            match auth {
                None => {
                    ...
                    let mut response = Response::new(serde_json::to_string(&result).unwrap().into());
                ...
                Some(_) => {
                    ...
                    future::ok(Response::new(serde_json::to_string(&result).unwrap().into()))
                }
            }
        };
    ...
    fn get_operation_status_success() {
        ...
        let handler = move |req: Request<Body>| {
            ...
            future::ok(
                Response::new(serde_json::to_string(&serializable).unwrap().into()),
            )
        };
        let client = Client::new(
            handler,
