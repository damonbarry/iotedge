...
use hyper::{Uri as HyperUri, client::connect::Destination};
use url::Url;
...
mod tests {
    ...
    fn uri_host_scheme() {
        ...
        assert!(Uri::get_pipe_path_from_parts(uri.scheme_part().unwrap().as_str(), uri.host().unwrap()).is_err());
    }
    ...
    #[test]
    fn uri_host() {
        let uri: HyperUri = "npipe://5c5c2e5c706970655c646f636b65725f656e67696e65/containers/json?all=true"
            .parse()
            .unwrap();
        assert_eq!(
            &Uri::get_pipe_path_from_parts(uri.scheme_part().unwrap().as_str(), uri.host().unwrap()).unwrap(),
            "\\\\.\\pipe\\docker_engine"
        );
    }
}
