...
use hyper::{Uri, server::conn::Http};
        ...
        match settings.provisioning() {
            ...
            Provisioning::Dps(dps) => {
                ...
                let (key_store, provisioning_result, root_key, runtime) =
                    dps_provision(&dps, hyper_client.clone(), dps_path, runtime, &mut tokio_runtime)?;
                info!("Finished provisioning edge device.");
                start_api(
                    &settings,
                    hyper_client,
    ...
    C: MasterEncryptionKey + CreateCertificate,
{
    ...
    if reconfig_reqd {
        reconfigure(subdir_path, filename, settings, runtime, crypto, tokio_runtime)?;
    }
    Ok(())
}
        ...
        + Sync,
{
    ...
    tokio_runtime.block_on_all(
        shutdown.select(services)
        .then(|result| match result {
            Ok(((), _)) => Ok(()),
    ...
    HC: 'static + ClientImpl + Send + Sync,
{
    ...
    .and_then(move |service| {
        let run = Http::new()
            .bind_url(url.clone(), service).map_err(failure::Fail::compat)?
