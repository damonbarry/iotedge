...
use typed_headers::{ContentLength, ContentType, HeaderMapExt, mime};
...
fn image_pull_handler(req: Request<Body>) -> Box<Future<Item = Response<Body>, Error = HyperError> + Send> {
...
fn image_pull_succeeds() {
    ...
    let server =
        run_tcp_server("127.0.0.1", port, image_pull_handler)
        .map_err(|err| eprintln!("{}", err));

    let mri = DockerModuleRuntime::new(&Url::parse(&format!("http://localhost:{}/", port)).unwrap()).unwrap();
...
#[cfg(unix)]
#[cfg_attr(feature = "cargo-clippy", allow(needless_pass_by_value))]
fn image_pull_with_creds_handler(req: Request<Body>) -> Box<Future<Item = Response<Body>, Error = HyperError> + Send> {
    // verify that path is /images/create and that the "fromImage" query
    // parameter has the image name we expect
    assert_eq!(req.uri().path(), "/images/create");
...
fn image_pull_with_creds_succeeds() {
    let port = get_unused_tcp_port();
    let server =
        run_tcp_server("127.0.0.1", port, image_pull_with_creds_handler)
        .map_err(|err| eprintln!("{}", err));
...
#[cfg_attr(feature = "cargo-clippy", allow(needless_pass_by_value))]
fn image_remove_handler(req: Request<Body>) -> Box<Future<Item = Response<Body>, Error = HyperError> + Send> {
...
fn image_remove_succeeds() {
    let port = get_unused_tcp_port();
    let server =
        run_tcp_server("127.0.0.1", port, image_remove_handler)
        .map_err(|err| eprintln!("{}", err));

    let mut mri = DockerModuleRuntime::new(&Url::parse(&format!("http://localhost:{}/", port)).unwrap()).unwrap();
...
fn container_create_handler(req: Request<Body>) -> Box<Future<Item = Response<Body>, Error = HyperError> + Send> {
...
fn container_create_succeeds() {
    let port = get_unused_tcp_port();
    let server =
        run_tcp_server("127.0.0.1", port, container_create_handler)
        .map_err(|err| eprintln!("{}", err));
...
fn container_start_handler(req: Request<Body>) -> Box<Future<Item = Response<Body>, Error = HyperError> + Send> {
    assert_eq!(req.method(), &Method::POST);
...
fn container_start_succeeds() {
    let port = get_unused_tcp_port();
    let server =
        run_tcp_server("127.0.0.1", port, container_start_handler)
        .map_err(|err| eprintln!("{}", err));
...
fn container_stop_handler(req: Request<Body>) -> Box<Future<Item = Response<Body>, Error = HyperError> + Send> {
    assert_eq!(req.method(), &Method::POST);
...
fn container_stop_succeeds() {
    let port = get_unused_tcp_port();
    let server =
        run_tcp_server("127.0.0.1", port, container_stop_handler)
        .map_err(|err| eprintln!("{}", err));
...
fn container_stop_with_timeout_handler(req: Request<Body>) -> Box<Future<Item = Response<Body>, Error = HyperError> + Send> {
    assert_eq!(req.method(), &Method::POST);
    assert_eq!(req.uri().path(), "/containers/m1/stop");
...
fn container_remove_handler(req: Request<Body>) -> Box<Future<Item = Response<Body>, Error = HyperError> + Send> {
    assert_eq!(req.method(), &Method::DELETE);
...
fn container_remove_succeeds() {
    let port = get_unused_tcp_port();
    let server =
        run_tcp_server("127.0.0.1", port, container_remove_handler)
        .map_err(|err| eprintln!("{}", err));
...
fn container_list_handler(req: Request<Body>) -> Box<Future<Item = Response<Body>, Error = HyperError> + Send> {
...
fn container_list_succeeds() {
    let port = get_unused_tcp_port();
    let server =
        run_tcp_server("127.0.0.1", port, container_list_handler)
        .map_err(|err| eprintln!("{}", err));
...
fn container_logs_handler(req: Request<Body>) -> Box<Future<Item = Response<Body>, Error = HyperError> + Send> {
    assert_eq!(req.method(), &Method::GET);
...
fn container_logs_succeeds() {
    let port = get_unused_tcp_port();
    let server =
        run_tcp_server("127.0.0.1", port, container_logs_handler)
        .map_err(|err| eprintln!("{}", err));
