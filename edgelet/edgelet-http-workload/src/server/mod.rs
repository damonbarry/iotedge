...
impl WorkloadService {
    pub fn new<K, H, M>(key_store: &K, hsm: H, runtime: &M) -> impl Future<Item = Self, Error = failure::Error>
    where
        ...
        M::Logs: Into<Body>,
    {
        ...
        router.new_service().map(|inner| WorkloadService { inner }).map_err(failure::Error::from_boxed_compat)
    }
}
